...
import { Invoice, InvoiceStatus, LineItem, NewInvoice } from '@/types/finance';

type InvoiceModalProps = {
  visible: boolean;
  onClose: () => void;
  onSave: (item: NewInvoice | Partial<Invoice>) => Promise<{ ok: boolean; message?: string }>;
...
export function InvoiceModal({
  ...
  useEffect(() => {
    if (editingItem) {
      ...
      setDueDate(new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]); // Default 14 days
      setStatus('Draft');
  ...
  function validate(): boolean {
    ...
    if (!dueDate.trim()) {
      ...
      nextErrors.dueDate = 'Due date format must be YYYY-MM-DD';
    }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const itemData = {
      invoiceId, // Let backend generate if empty
    ...
    if (res.ok) {
      ...
      setErrors({ form: res.message || 'Failed to save invoice.' });
    }
  }
    ...
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose}>
      <View style={styles.root}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{editingItem ? `Edit Invoice (${editingItem.invoiceId})` : 'New Invoice Draft'}</Text>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          {/* Block 2: Dates & Taxes */}
          <Text style={styles.sectionHeader}>Invoice Setup & Taxes</Text>
          <View style={styles.card}>
          ...
          <Text style={styles.sectionHeader}>Invoice Status</Text>
            ...
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveBtnText}>
                  {editingItem ? 'Save Invoice' : 'Create Invoice'}
                </Text>
              </>
            )}
          </Pressable>
