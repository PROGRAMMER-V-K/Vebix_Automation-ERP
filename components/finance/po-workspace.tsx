...
import { PurchaseOrder, LineItem } from '@/types/finance';
import { generateNextPurchaseOrderId } from '@/lib/finance-firestore';

// Helper to generate the print HTML matching the Vebix sample PO exactly
export function generatePOHtml(po: Omit<PurchaseOrder, 'id' | 'createdAt' | 'updatedAt' | 'updatedBy'>): string {
    ...
    <body>
      <div class="container">
        ...
        <div class="header-top">
          <div>
            <div class="company-title">${po.shipToName || 'VEBIX AUTOMATION'}</div>
            <div class="company-details">
              Address: ${po.shipToAddress || ''}<br>
              Contact: ${po.shipToContact || ''} | Email: ${po.shipToEmail || ''}<br>
          ...
          <div class="po-title-block">
            ...
            <table class="po-meta-table">
              ...
              <tr>
                ...
                <td style="font-weight: bold;">${po.poId}</td>
              </tr>
            </table>
          </div>
        </div>
        ...
        <div class="supplier-ship-to">
          <div class="supplier-block">
            <div class="block-header">Supplier</div>
            <div class="block-content">
              <strong>${po.supplierName || '—'}</strong><br>
              ${po.supplierAddress ? po.supplierAddress.replace(/\n/g, '<br>') : '—'}<br>
              ${po.supplierContact ? 'Contact: ' + po.supplierContact : ''} ${po.supplierEmail ? '| Email: ' + po.supplierEmail : ''}
              ${po.supplierGst ? '<br>GST NO.: ' + po.supplierGst : ''}
            </div>
          </div>
          <div class="ship-to-block">
            <div class="block-header">Ship To</div>
            <div class="block-content">
              <strong>${po.shipToName}</strong><br>
              ${po.shipToAddress ? po.shipToAddress.replace(/\n/g, '<br>') : ''}<br>
              Contact: ${po.shipToContact} | Email: ${po.shipToEmail}
            </div>
          </div>
        </div>
        ...
        <table class="logistics-table">
          ...
          <tbody>
            <tr>
              <td>${po.requisitioner || '&nbsp;'}</td>
              <td>${po.shipVia || '&nbsp;'}</td>
              <td>${po.fob || '&nbsp;'}</td>
              <td>${po.shippingTerms || '&nbsp;'}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="footer-section">
          <div class="comments-block">
            ...
            <div class="comments-content">${po.comments ? po.comments.replace(/\n/g, '<br>') : 'To be delivered free of cost at above address<br>Delivery period: 4-5 weeks'}</div>
          </div>
          <div class="pricing-block">
            <table class="pricing-table">
              <tr>
                <td class="pricing-label">SUBTOTAL</td>
                <td class="pricing-val">${formatCurrency(po.subtotal)}</td>
              </tr>
              <tr>
                <td class="pricing-label">TAX (${po.taxRate}%)</td>
                <td class="pricing-val">${formatCurrency(po.taxAmount)}</td>
              </tr>
              <tr>
                <td class="pricing-label">SHIPPING</td>
                <td class="pricing-val">${po.shippingAmount > 0 ? formatCurrency(po.shippingAmount) : '-'}</td>
              </tr>
              <tr>
                <td class="pricing-label">OTHER</td>
                <td class="pricing-val">${po.otherAmount > 0 ? formatCurrency(po.otherAmount) : '-'}</td>
              </tr>
              <tr class="total-row">
                <td style="border: 1px solid #1e3a5f;">TOTAL</td>
                <td style="border: 1px solid #1e3a5f; text-align: right;">₹ ${formatCurrency(po.total)}</td>
              </tr>
            </table>
          </div>
        </div>
...
type PurchaseOrderWorkspaceProps = {
  ...
  onClose: () => void;
  onSave: (poData: any) => Promise<{ ok: boolean; message?: string }>;
};

export function PurchaseOrderWorkspace({ editingItem, onClose, onSave }: PurchaseOrderWorkspaceProps) {
  ...
  // --- Purchase Order Form Fields State ---
  ...
  const [shipToName, setShipToName] = useState('VEBIX AUTOMATION LLP');
  ...
  const [shipToEmail, setShipToEmail] = useState('vebixauto@gmail.com');
  ...
  const [comments, setComments] = useState(
    'To be delivered free of cost at above address\nDelivery period: 4-5 weeks\nTest certificate, Certificate of Conformance to be provided along with delivery or via Email'
  );

  // Initialize fields on mount or when editing item shifts
  useEffect(() => {
    if (editingItem) {
      setPoId(editingItem.poId);
      setDate(editingItem.date);
      setSupplierName(editingItem.supplierName);
      setSupplierAddress(editingItem.supplierAddress);
      setSupplierContact(editingItem.supplierContact);
      setSupplierEmail(editingItem.supplierEmail);
      setSupplierGst(editingItem.supplierGst);

      setShipToName(editingItem.shipToName);
      setShipToAddress(editingItem.shipToAddress);
      setShipToContact(editingItem.shipToContact);
      setShipToEmail(editingItem.shipToEmail);

      setRequisitioner(editingItem.requisitioner);
      setShipVia(editingItem.shipVia);
      setFob(editingItem.fob);
      setShippingTerms(editingItem.shippingTerms);

      setItems(editingItem.items);
      setTaxRate(editingItem.taxRate);
      setShippingAmount(editingItem.shippingAmount);
      setOtherAmount(editingItem.otherAmount);
      setComments(editingItem.comments);
    } else {
      // Auto generate PO ID
      generateNextPurchaseOrderId().then((nextId) => setPoId(nextId));
  ...
  const total = useMemo(() => {
    return subtotal + taxAmount + shippingAmount + otherAmount;
  }, [subtotal, taxAmount, shippingAmount, otherAmount]);
  ...
  const getPoData = (): Omit<PurchaseOrder, 'id' | 'createdAt' | 'updatedAt' | 'updatedBy'> => {
    return {
      poId,
      date,
      supplierName,
  ...
  const handlePrint = async () => {
    if (!supplierName.trim()) {
      Alert.alert('Validation Error', 'Please specify a Supplier Name.');
      return;
    }
    const htmlContent = generatePOHtml(getPoData());
    try {
      if (Platform.OS === 'web') {
  ...
  const handleSave = async () => {
    ...
    const res = await onSave(poData);
    setSaving(false);

    if (res.ok) {
      onClose();
    } else {
      Alert.alert('Error Saving PO', res.message || 'Database write failed.');
    }
  };
  ...
  const renderFormInputs = () => (
    <ScrollView style={styles.formScroll} showsVerticalScrollIndicator={false}>
      <Text style={styles.formSectionTitle}>1. Purchase Order Metadata</Text>
      <View style={styles.formRow}>
        <View style={styles.formField}>
          <Text style={styles.inputLabel}>PO ID (Auto-Generated)</Text>
          <TextInput style={styles.textInput} value={poId} onChangeText={setPoId} />
        </View>
        <View style={styles.formField}>
      ...
      <Text style={styles.formSectionTitle}>2. Supplier Details</Text>
      <View style={styles.formField}>
        <Text style={styles.inputLabel}>Supplier Name</Text>
        <TextInput
          style={styles.textInput}
          placeholder="e.g. REL-FZ LLC MOSCOW"
          value={supplierName}
          onChangeText={setSupplierName}
        />
      </View>
      <View style={styles.formField}>
        <Text style={styles.inputLabel}>Supplier Address</Text>
        <TextInput
      ...
      <View style={styles.formRow}>
        <View style={styles.formField}>
          ...
          <TextInput style={styles.textInput} placeholder="e.g. +7 999 123" value={supplierContact} onChangeText={setSupplierContact} />
        </View>
        <View style={styles.formField}>
          <Text style={styles.inputLabel}>Email Address</Text>
          <TextInput style={styles.textInput} placeholder="supplier@email.com" value={supplierEmail} onChangeText={setSupplierEmail} />
        </View>
      </View>
      <View style={styles.formField}>
        <Text style={styles.inputLabel}>GST Number (If applicable)</Text>
        <TextInput style={styles.textInput} placeholder="e.g. 27AZBPA0013H1Z8" value={supplierGst} onChangeText={setSupplierGst} />
      </View>

      <Text style={styles.formSectionTitle}>3. Shipping & Delivery Destination</Text>
      <View style={styles.formField}>
        <Text style={styles.inputLabel}>Ship To Name</Text>
        <TextInput style={styles.textInput} value={shipToName} onChangeText={setShipToName} />
      </View>
      <View style={styles.formField}>
        <Text style={styles.inputLabel}>Ship To Address</Text>
        <TextInput
          style={[styles.textInput, styles.multilineInput]}
          multiline
          value={shipToAddress}
      ...
      <View style={styles.formRow}>
        <View style={styles.formField}>
          <Text style={styles.inputLabel}>Ship To Contact</Text>
          <TextInput style={styles.textInput} value={shipToContact} onChangeText={setShipToContact} />
        </View>
        <View style={styles.formField}>
          <Text style={styles.inputLabel}>Ship To Email</Text>
      ...
      <View style={styles.a4Page}>
        ...
        <View style={styles.poHeader}>
          ...
          <View style={{ flex: 0.8, alignItems: 'flex-end' }}>
            <Text style={styles.previewPoTitle}>PURCHASE ORDER</Text>
            <View style={styles.previewMetaGrid}>
              ...
              <View style={[styles.previewMetaRow, { borderBottomWidth: 0 }]}>
                <Text style={styles.previewMetaLabel}>PO NO.</Text>
                <Text style={[styles.previewMetaValue, { fontWeight: '700' }]}>{poId || '—'}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        {/* Supplier / Ship To Columns */}
        <View style={styles.previewAddressesRow}>
          <View style={styles.previewAddressBlock}>
            <View style={styles.previewBlockHeader}>
              <Text style={styles.previewBlockHeaderText}>SUPPLIER</Text>
            </View>
            <View style={styles.previewBlockContent}>
              <Text style={styles.boldText} numberOfLines={1}>{supplierName || '—'}</Text>
              <Text style={styles.smallText}>{supplierAddress || '—'}</Text>
              {supplierContact || supplierEmail ? (
                <Text style={styles.smallText}>{supplierContact} {supplierEmail}</Text>
              ) : null}
              {supplierGst ? (
                <Text style={styles.smallText}>GST: {supplierGst}</Text>
              ) : null}
            </View>
          </View>
        ...
        <View style={styles.previewLogisticsStrip}>
          ...
          <View style={styles.logisticsCol}>
            <Text style={styles.logisticsHeader}>FOB</Text>
            <Text style={styles.logisticsValue} numberOfLines={1}>{fob || '—'}</Text>
          </View>
          <View style={[styles.logisticsCol, { borderRightWidth: 0, flex: 1.5 }]}>
        ...
        <View style={styles.previewItemsTable}>
          ...
          {items.length === 0 ? (
            <View style={styles.previewTableEmpty}>
              <Text style={styles.emptyTableText}>No items added to purchase order yet.</Text>
            </View>
          ) : (
            items.map((item, index) => {
        ...
        <View style={styles.previewFooterGrid}>
          <View style={styles.previewCommentsBlock}>
            ...
            <View style={styles.previewCommentsContent}>
              <Text style={styles.previewCommentsText}>{comments || 'To be delivered free of cost...'}</Text>
            </View>
          </View>
          ...
          <View style={styles.previewTotalsBlock}>
            ...
            <View style={styles.totalItemRow}>
              <Text style={styles.totalItemLabel}>TAX ({taxRate}%)</Text>
              <Text style={styles.totalItemValue}>{formatCurrency(taxAmount)}</Text>
            </View>
            <View style={styles.totalItemRow}>
      ...
      <View style={styles.workspaceHeader}>
        <View style={styles.headerTitleRow}>
          ...
          <Text style={styles.workspaceTitle}>
            {editingItem ? 'Edit Purchase Order' : 'Create Purchase Order'}
          </Text>
        </View>
        ...
        <View style={styles.workspaceHeaderActions}>
          ...
          >
            ...
            <Text style={styles.saveBtnText}>{saving ? 'Saving...' : 'Save PO'}</Text>
          </Pressable>
        </View>
      </View>
      ...
      {isDesktop ? (
        ...
        <View style={styles.desktopLayoutRow}>
          <View style={styles.leftFormColumn}>
            <Text style={styles.colTitle}>Purchase Order Form Details</Text>
            {renderFormInputs()}
          </View>
