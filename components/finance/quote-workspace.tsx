...
import { generateNextQuotationId } from '@/lib/finance-firestore';
import { LineItem, Quotation } from '@/types/finance';

// Helper to generate the print HTML matching th/Vebix sample PO style exactly but for Quotes
export function generateQuoteHtml(quote: Omit<Quotation, 'id' | 'createdAt' | 'updatedAt' | 'updatedBy'>): string {
  const formatCurrency = (val: number) => {
    return val.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  };

  const rowsHtml = quote.items.map((item, idx) => `
    <tr>
    ...
    <body>
      <div class="container">
        ...
        <div class="header-top">
          <div>
            <div class="company-title">${quote.shipToName || 'Vebix AUTOMATION'}</div>
            <div class="company-details">
              Address: ${quote.shipToAddress || ''}<br>
              Contact: ${quote.shipToContact || ''} | Email: ${quote.shipToEmail || ''}<br>
          ...
          <div class="quote-title-block">
            ...
            <table class="quote-meta-table">
              ...
              <tr>
                <td class="quote-meta-label">QUOTE NO.</td>
                <td style="font-weight: bold;">${quote.quoteId}</td>
              </tr>
            </table>
          </div>
        </div>
        ...
        <div class="client-ship-to">
          <div class="client-block">
            ...
            <div class="block-content">
              ...
              ${quote.clientAddress ? quote.clientAddress.replace(/\n/g, '<br>') : '—'}<br>
              ${quote.clientContact ? 'Contact: ' + quote.clientContact : ''} ${quote.clientEmail ? '| Email: ' + quote.clientEmail : ''}
            </div>
          </div>
          <div class="ship-to-block">
            <div class="block-header">Company / Ship To</div>
            <div class="block-content">
              <strong>${quote.shipToName}</strong><br>
              ${quote.shipToAddress ? quote.shipToAddress.replace(/\n/g, '<br>') : ''}<br>
              Contact: ${quote.shipToContact} | Email: ${quote.shipToEmail}
            </div>
          </div>
        </div>
        ...
        <table class="logistics-table">
          ...
          <tbody>
            <tr>
              <td>${quote.requisitioner || '&nbsp;'}</td>
              <td>${quote.shipVia || '&nbsp;'}</td>
              <td>${quote.shippingTerms || '&nbsp;'}</td>
              <td><strong>${quote.validUntil || '&nbsp;'}</strong></td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="footer-section">
          <div class="comments-block">
            ...
            <div class="comments-content">${quote.comments ? quote.comments.replace(/\n/g, '<br>') : 'Validity: 30 days from date of quote<br>Delivery period: 3-4 weeks'}</div>
          </div>
          <div class="pricing-block">
            <table class="pricing-table">
              <tr>
                <td class="pricing-label">SUBTOTAL</td>
                <td class="pricing-val">${formatCurrency(quote.subtotal)}</td>
              </tr>
              <tr>
                <td class="pricing-label">TAX (${quote.gstRate}%)</td>
                <td class="pricing-val">${formatCurrency(quote.gstAmount)}</td>
              </tr>
              <tr>
                <td class="pricing-label">SHIPPING</td>
                <td class="pricing-val">${quote.shippingAmount && quote.shippingAmount > 0 ? formatCurrency(quote.shippingAmount) : '-'}</td>
              </tr>
              <tr>
                <td class="pricing-label">OTHER</td>
                <td class="pricing-val">${quote.otherAmount && quote.otherAmount > 0 ? formatCurrency(quote.otherAmount) : '-'}</td>
              </tr>
              <tr class="total-row">
                <td style="border: 1px solid #1e3a5f;">TOTAL</td>
                <td style="border: 1px solid #1e3a5f; text-align: right;">₹ ${formatCurrency(quote.total)}</td>
              </tr>
            </table>
          </div>
        </div>
...
type QuotationWorkspaceProps = {
  ...
  onClose: () => void;
  onSave: (quoteData: any) => Promise<{ ok: boolean; message?: string }>;
};

export function QuotationWorkspace({ editingItem, onClose, onSave }: QuotationWorkspaceProps) {
  ...
  const [shipToEmail, setShipToEmail] = useState('Vebixauto1980@gmail.com');
  ...
  useEffect(() => {
    if (editingItem) {
      ...
      setComments(editingItem.comments || 'Validity: 30 days from date of quote');
    } else {
      // Auto generate Quote ID
      generateNextQuotationId().then((nextId) => setQuoteId(nextId));
  ...
  const handlePrint = async () => {
    ...
    const htmlContent = generateQuoteHtml(getQuoteData());
    try {
      if (Platform.OS === 'web') {
  ...
  const handleSave = async () => {
    ...
    const res = await onSave(quoteData);
    setSaving(false);

    if (res.ok) {
      onClose();
    } else {
      Alert.alert('Error Saving Quote', res.message || 'Database write failed.');
    }
  };
  ...
  const renderFormInputs = () => (
    <ScrollView style={styles.formScroll} showsVerticalScrollIndicator={false}>
      <Text style={styles.formSectionTitle}>1. Quotation Metadata</Text>
      <View style={styles.formRow}>
        <View style={styles.formField}>
          <Text style={styles.inputLabel}>Quote ID (Auto-Generated)</Text>
          <TextInput style={styles.textInput} value={quoteId} onChangeText={setQuoteId} />
        </View>
        <View style={styles.formField}>
      ...
      <View style={styles.formField}>
        ...
        <TextInput style={styles.textInput} value={validUntil} onChangeText={setValidUntil} />
      </View>

      <Text style={styles.formSectionTitle}>2. Client Details</Text>
      <View style={styles.formField}>
      ...
      <View style={styles.a4Page}>
        ...
        <View style={styles.poHeader}>
          ...
          <View style={{ flex: 0.8, alignItems: 'flex-end' }}>
            <Text style={styles.previewPoTitle}>QUOTATION</Text>
            <View style={styles.previewMetaGrid}>
              ...
              <View style={[styles.previewMetaRow, { borderBottomWidth: 0 }]}>
                <Text style={styles.previewMetaLabel}>QUOTE NO.</Text>
                <Text style={[styles.previewMetaValue, { fontWeight: '700' }]}>{quoteId || '—'}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.previewAddressesRow}>
          <View style={styles.previewAddressBlock}>
            <View style={styles.previewBlockHeader}>
              <Text style={styles.previewBlockHeaderText}>CLIENT</Text>
            </View>
          ...
          <View style={styles.previewAddressBlock}>
            <View style={styles.previewBlockHeader}>
              <Text style={styles.previewBlockHeaderText}>COMPANY / SHIP TO</Text>
            </View>
            <View style={styles.previewBlockContent}>
        ...
        <View style={styles.previewLogisticsStrip}>
          ...
          <View style={[styles.logisticsCol, { borderRightWidth: 0, flex: 1.2 }]}>
            <Text style={styles.logisticsHeader}>VALID UNTIL</Text>
            <Text style={[styles.logisticsValue, { fontWeight: '700' }]} numberOfLines={1}>{validUntil || '—'}</Text>
          </View>
        </View>
        ...
        <View style={styles.previewItemsTable}>
          ...
          {items.length === 0 ? (
            <View style={styles.previewTableEmpty}>
              <Text style={styles.emptyTableText}>No items added to quotation yet.</Text>
            </View>
          ) : (
            items.map((item, index) => {
        ...
        <View style={styles.previewFooterGrid}>
          <View style={styles.previewCommentsBlock}>
            ...
            <View style={styles.previewCommentsContent}>
              <Text style={styles.previewCommentsText}>{comments || 'Validity: 30 days...'}</Text>
            </View>
          </View>
      ...
      <View style={styles.workspaceHeader}>
        <View style={styles.headerTitleRow}>
          ...
          <Text style={styles.workspaceTitle}>
            {editingItem ? 'Edit Quotation' : 'Create Quotation'}
          </Text>
        </View>
        ...
        <View style={styles.workspaceHeaderActions}>
          ...
          >
            ...
            <Text style={styles.saveBtnText}>{saving ? 'Saving...' : 'Save Quote'}</Text>
          </Pressable>
        </View>
      </View>
      ...
      {isDesktop ? (
        <View style={styles.desktopLayoutRow}>
          <View style={styles.leftFormColumn}>
            <Text style={styles.colTitle}>Quotation Form Details</Text>
            {renderFormInputs()}
          </View>
